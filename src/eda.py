...
def create_textblob_plot(polarity_output, target, outdir):
    '''
    ...
    sns.lineplot(data=polarity_output)
    plt.xlabel('# Days Before and After Cancellation')
    plt.ylabel("Polarity")
...
def plotPolarity(meanPolar, target, outdir):
    '''
    produces line plot tracking polarity of tweets relating to
...
def calculate_stats(data_dict, test=False):
    if test == False:
        ...
        for indiv in artist_names:
            ...
            # polarity_df = convert_dates(polarity_data)
